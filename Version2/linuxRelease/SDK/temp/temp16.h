/*
该版本使用了在temp15.h的基础上增加了
无货物机器人的购买策略，但分数反而更低
if(dis_high < dis_normal + 50)
但由于可修改距离，设定距离为0时，可以退回原版本
...
int Manager::find_recenttable(int robot_id)
{
    ...
    if(m_robots[robot_id].objectID==0)//如果没有带，此刻空闲可以取
    {
        ...
        for(int i=0;i<m_amount_workTable;i++) {
            if (m_worktables[i].classID == 7 && m_worktables[i].sta_produce && m_worktables[i].robot_link >= robot_id) {
