...
struct Robot{   // 机器人属性
    ...
    char order = 0; //是否携带指令，0表示没有指令，1表示正在执行指令
...
class Manager{
    public:
         ...
         */
        void workClass(Robot m_robot, Worktable m_worktable);

        /**
         * @brief 工作台功能
         * 
         * @param t_robot 机器人
         * @param t_worktable 工作台
         */
        void work_1_func(Robot t_robot, Worktable t_worktable);
        void work_2_func(Robot t_robot, Worktable t_worktable);
        void work_3_func(Robot t_robot, Worktable t_worktable);
        void work_4_func(Robot t_robot, Worktable t_worktable);
        void work_5_func(Robot t_robot, Worktable t_worktable);
        void work_6_func(Robot t_robot, Worktable t_worktable);
        void work_7_func(Robot t_robot, Worktable t_worktable);
        void work_8_func(Robot t_robot, Worktable t_worktable);
        void work_9_func(Robot t_robot, Worktable t_worktable);
...
/**********工作台类**********/
void Manager::workClass(Robot m_robot, Worktable m_worktable)
{
    uint8_t classID;
    switch(m_worktable.classID)
    {
        /*
        case 1:
            work_1_func(Robot t_robot, Worktable t_worktable);
            break;
        case 2:
            work_2_func(worktable);
            break;
        case 3:
            work_3_func(worktable);
            break;
        case 4:
            work_4_func(worktable);
            break;
        case 5:
            work_5_func(worktable);
            break;
        case 6:
            work_6_func(worktable);
            break;
        case 7:
            work_7_func(worktable);
            break;
        case 8:
            work_8_func(worktable);
            break;
        case 9:
            work_9_func(worktable);
            break; */
        default:
            return;
    }
...
/**********1号工作台功能**********/
void Manager::work_1_func(Robot t_robot, Worktable t_worktable)
{
    int frameid = m_frameID;
    if(t_worktable.sta_produce)
    {
        control(t_robot.ID, ACT_BUY);
        /*待增加代码：计算剩余生产帧数*/
    }
}

void Manager::work_2_func(Robot t_robot, Worktable t_worktable)
{
    int frameid = m_frameID;
    if(t_worktable.sta_produce)
    {
...
void Manager::work_3_func(Robot t_robot, Worktable t_worktable)
{
    int frameid = m_frameID;
    if(t_worktable.sta_produce)
    {
...
void Manager::planner()
{
    ...
    {
        if(m_robots[cnt_robot].objectID == 7)
        {
            for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)
            {
                ...
                {
                    set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
...
{   
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //先判断一遍所有7类工作台是否有产品
    {
        if(m_worktables[cnt_table].classID == 7)
        {
            if(m_worktables[cnt_table].sta_produce == 1)
            {
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)              //这里没有寻找最近的机器人，只是按顺序找未携带物品的机器人
                {
                    if(m_robots[cnt_robot].objectID == 0)
                    {
                        set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
                        control(m_robots[cnt_robot].ID, ACT_BUY);
                        break;
                    }
                }
            }
    ...
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //如果所有7类工作台都没有产品
    {
        
        if(m_worktables[cnt_table].classID == 7)        //按顺序找到类别为7的工作台
        {
                ...
                if((temp_material & (1 << 6)) == 0)      //表示缺少6号原材料
                {
                    ...
                    {
                        if(m_robots[cnt_robot].objectID == 6)
                        {
                            has_product6 = 1;
                            set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
                ...
                else if((temp_material & (1 << 5)) == 0)      //表示缺少5号原材料
                {
                    int has_product5 = 0;        
                    for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)          //搜索四个机器人中是否有携带5号原材料的
                    {
                        if(m_robots[cnt_robot].objectID == 5)
                        {
                            has_product5 = 1;
                            set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
                            control(m_robots[cnt_robot].ID, ACT_SELL);
                ...
                else if((temp_material & (1 << 4)) == 0)      //表示缺少4号原材料
                {
                    int has_product4 = 0;        
                    for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)          //搜索四个机器人中是否有携带4号原材料的
                    {
                        if(m_robots[cnt_robot].objectID == 4)
                        {
                            has_product4 = 1;
                            set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
                            control(m_robots[cnt_robot].ID, ACT_SELL);
                ...
                else if((temp_material &(1 << 6)) && (temp_material &(1 << 5)) && (temp_material &(1 << 4)))            //如果所有材料均有，则跳过这个7类工作台，寻找下一个7类工作台
                {
                    continue;
                }
...
{
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //先判断一遍所有6类工作台是否有产品
    {
        if(m_worktables[cnt_table].classID == 6)
        {
            if(m_worktables[cnt_table].sta_produce == 1)
            {
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)              //这里没有寻找最近的机器人，只是按顺序找未携带物品的机器人
    ...
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //如果所有6类工作台都没有产品
    {
        
        if(m_worktables[cnt_table].classID == 6)        //按顺序找到类别为6的工作台
        {
            uint16_t temp_material;
            temp_material = m_worktables[cnt_table].sta_material;
            if((temp_material & (1 << 3)) == 0)      //表示缺少3号原材料
            {
                ...
                {
                    if(m_robots[cnt_robot].objectID == 3)
                    {
                        has_product3 = 1;
                        set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
                        control(m_robots[cnt_robot].ID, ACT_SELL);
            ...
            else if((temp_material & (1 << 2)) == 0)      //表示缺少2号原材料
            {
                int has_product2 = 0;        
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)          //搜索四个机器人中是否有携带2号原材料的
                {
                    if(m_robots[cnt_robot].objectID == 2)
                    {
                        has_product2 = 1;
                        set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
                        control(m_robots[cnt_robot].ID, ACT_SELL);
            ...
            else if((temp_material &(1 << 3)) && (temp_material &(1 << 2)))         //如果所有材料均有，则跳过这个6类工作台，寻找下一个6类工作台
            {
                 continue;
            }  
        }
...
{
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //先判断一遍所有5类工作台是否有产品
    {
        if(m_worktables[cnt_table].classID == 5)
        {
            if(m_worktables[cnt_table].sta_produce == 1)
            {
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)              //这里没有寻找最近的机器人，只是按顺序找未携带物品的机器人
    ...
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //如果所有5类工作台都没有产品
    {
        
        if(m_worktables[cnt_table].classID == 5)        //按顺序找到类别为5的工作台
        {
            uint16_t temp_material;
            temp_material = m_worktables[cnt_table].sta_material;
            if((temp_material & (1 << 3)) == 0)      //表示缺少3号原材料
            ...
            else if((temp_material & (1 << 1)) == 0)      //表示缺少1号原材料
            {
                int has_product1 = 0;        
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)          //搜索四个机器人中是否有携带1号原材料的
                {
                    if(m_robots[cnt_robot].objectID == 1)
                    {
                        has_product1 = 1;
                        set_Pointv4(m_robots[cnt_robot].ID, m_worktables[cnt_table].loc.x,  m_worktables[cnt_table].loc.y);
                        control(m_robots[cnt_robot].ID, ACT_SELL);
            ...
            else if((temp_material &(1 << 3)) && (temp_material &(1 << 1)))         //如果所有材料均有，则跳过这个6类工作台，寻找下一个6类工作台
            {
                 continue;
            }  
        }
...
{
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //先判断一遍所有4类工作台是否有产品
    {
        if(m_worktables[cnt_table].classID == 4)
        {
            if(m_worktables[cnt_table].sta_produce == 1)
            {
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)              //这里没有寻找最近的机器人，只是按顺序找未携带物品的机器人
    ...
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //如果所有4类工作台都没有产品
    {
        
        if(m_worktables[cnt_table].classID == 4)        //按顺序找到类别为4的工作台
        {
            uint16_t temp_material;
            temp_material = m_worktables[cnt_table].sta_material;
            ...
            else if((temp_material &(1 << 2)) && (temp_material &(1 << 1)))         //如果所有材料均有，则跳过这个6类工作台，寻找下一个6类工作台
            {
                 continue;
            }  
        }
...
{
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //先判断一遍所有3类工作台是否有产品
    {
        if(m_worktables[cnt_table].classID == 3)
        {
            if(m_worktables[cnt_table].sta_produce == 1)
            {
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)              //这里没有寻找最近的机器人，只是按顺序找未携带物品的机器人
...
{
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //先判断一遍所有2类工作台是否有产品
    {
        if(m_worktables[cnt_table].classID == 2)
        {
            if(m_worktables[cnt_table].sta_produce == 1)
            {
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)              //这里没有寻找最近的机器人，只是按顺序找未携带物品的机器人
...
{
    for(int cnt_table = 0; cnt_table < m_worktables.size(); cnt_table++)            //先判断一遍所有1类工作台是否有产品
    {
        if(m_worktables[cnt_table].classID == 1)
        {
            if(m_worktables[cnt_table].sta_produce == 1)
            {
                for(int cnt_robot = 0; cnt_robot < 4; cnt_robot++)              //这里没有寻找最近的机器人，只是按顺序找未携带物品的机器人
