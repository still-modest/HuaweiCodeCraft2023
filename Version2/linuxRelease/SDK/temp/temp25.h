...
int Manager::find_recenttable(int robot_id)
{
    ...
    if(m_robots[robot_id].objectID==0)//如果没有带，此刻空闲可以取
    {
        ...
        int extra_dis = 0;          //重要参数      当7类工作台检测到只缺其中一种原材料时，是否检测越过该距离搜索缺少材料，当为0时，不搜索。当为100时，全图搜索
    ...
    {
        int last_time3 = 0;         //重要参数      提前预测7.6.5.4类工作台提前出产结果的时间，并有该时间判断，该产品所缺少的原材料。当为0时，即为不预测。
...
float Manager::avoid_obs(int8_t robotID)
{
    ...
    float ang_dif_range = M_PI/2;        //重要参数     机器人角度差值
    float w = -1;
    for(int cnt_rob = 0; cnt_rob < 4; cnt_rob++)
    {
        if(cnt_rob == 3)
