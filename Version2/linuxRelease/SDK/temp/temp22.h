...
int Manager::find_recenttable(int robot_id)
{
    ...
    if(m_robots[robot_id].objectID==0)//如果没有带，此刻空闲可以取
    {
        ...
        //如果3、2、1类工作台有成果
        for(int i=0;i<m_amount_workTable;i++){
            if((m_worktables[i].classID==1 || m_worktables[i].classID==2 || m_worktables[i].classID==3) && m_worktables[i].sta_produce)//如果有货   && m_worktables[i].robot_link>=robot_id
            {
                if(class_1_enough && m_worktables[i].classID==1) continue;
                if(class_2_enough && m_worktables[i].classID==2) continue;
                if(class_3_enough && m_worktables[i].classID==3) continue;
...
//    {
    ...
    /*
    int distForR2R = 2;
    float ang_big = M_PI/6;
    float ang_small = M_PI/3;
    float ang_range = M_PI/2;
    for(int cnt_rob = 3; cnt_rob >= 0; cnt_rob--)
    {
        if(cnt_rob == 3)
        {
            for(int i = 0; i < 4; i++)
            ...
            for(int i=0; i<each_rob_dis3.size(); i++)
            {
                ...
                {
                    if(cnt_rob > each_rob_dis3[i].second)
                    {
        ...
        else if(cnt_rob == 2)
        {
            ...
            for(int i=0; i<each_rob_dis2.size(); i++)
            {
                ...
                {
                    if(cnt_rob > each_rob_dis2[i].second)
                    {
        ...
        else if(cnt_rob == 1)
        {
            ...
            for(int i=0; i<each_rob_dis1.size(); i++)
            {
                ...
                {
                    if(cnt_rob > each_rob_dis1[i].second)
                    {
        ...
        else if(cnt_rob == 0)
        {
            ...
            for(int i=0; i<each_rob_dis0.size(); i++)
            {
                ...
                {
                    if(cnt_rob > each_rob_dis0[i].second)
                    {
