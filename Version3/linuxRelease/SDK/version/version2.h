/*相比于version1修改了碰撞的优先级，没带物品的给带物品的躲避*/
...
vector<int> System::cost_fun(vector<pair<double,vector<int>>> dis_all, int robot_id)
{
    ...
    for(int i=0;i<lost_rate.size();i++)
    {
        ...
        if(glo_workBenches[lost_rate[i].second[0]].classID==2)            //当机器人寻路发现第一阶段的物品类别为2时
        {
            if(map_id==1)
            {
        ...
        if(glo_workBenches[lost_rate[i].second[0]].classID==3)              //当机器人寻路发现第一阶段的物品类别为3时
        {
            if(map_id==1)
            {
        ...
        if(glo_workBenches[lost_rate[i].second[0]].classID==6)
        {
            ...
            if(map_id==3)
            {
                monney_temp=monney_temp + 1000;         //需要修改值
